        ...
        <div className="flex justify-between items-center h-20">
          ...
          <div className="flex items-center shrink-0">
            <Link href="/" className="flex items-center gap-2 sm:gap-3 group">
              <div className="relative flex items-center justify-center bg-gradient-to-br from-blue-600 to-indigo-700 p-1.5 sm:p-2 rounded-xl group-hover:shadow-lg group-hover:shadow-blue-500/30 transition-all duration-300 group-hover:-rotate-2">
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className="text-white sm:w-6 sm:h-6">
              ...
              <div className="flex flex-col leading-none">
                <span className="text-lg sm:text-xl font-black tracking-tighter text-slate-900 dark:text-white uppercase italic">
                  ZK<span className="text-blue-600 not-italic font-extrabold">STORE</span>
                </span>
                <span className="hidden xs:block text-[8px] sm:text-[9px] font-bold text-gray-400 dark:text-slate-500 tracking-[0.1em] sm:tracking-[0.2em] uppercase mt-0.5">
                  Premium Store
                </span>
              </div>
            </Link>
          ...
          <div className="flex items-center gap-1 sm:gap-4">
            ...
            {/* Shopping Cart */}
            <Link href="/cart" className="relative p-2 sm:p-2.5 text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-full transition-all group">
              <ShoppingCart size={22} className="group-hover:text-blue-600 dark:group-hover:text-blue-500 transition-colors" />
              {cartCount > 0 && (
                <span className="absolute top-1 right-1 bg-blue-600 text-white text-[10px] font-bold rounded-full h-5 w-5 flex items-center justify-center border-2 border-white dark:border-slate-950">
                  {cartCount}
                </span>
              )}
            </Link>
